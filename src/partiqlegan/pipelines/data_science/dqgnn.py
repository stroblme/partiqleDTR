...
class HybridLayer(nn.Module):
    ...
    def __init__(self, n_in, n_hid, n_out, circuit_type, backend=None, shots=100, shift=np.pi/2):
        ...
        self.qnn1 = TwoLayerQNN(num_qubits=n_hid, quantum_instance=self.qi)
        self.initial_weights = 0.1 * (2 * q.utils.algorithm_globals.random.random(self.qnn1.num_weights) - 1)

        self.quantum_layer = TorchConnector(self.qnn1, initial_weights=self.initial_weights)
