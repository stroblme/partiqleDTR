        ...
        **kwargs,
        ...
        def encoding(qc, n_qubits, identifier):
            for i in range(n_qubits):
                ...
                px = (q.circuit.Parameter(f"{identifier}_rx_{i}") * energy * t.pi, i, f"{identifier}_rx_{i}")
                qc.rx(*px) 
                py = (q.circuit.Parameter(f"{identifier}_ry_{i}") * energy * t.pi, i, f"{identifier}_ry_{i}")
                qc.ry(*py) 
                pz = (q.circuit.Parameter(f"{identifier}_rz_{i}") * energy * t.pi, i) # rz does not accept identifier
                qc.rz(*pz) 
                # qc.ry(*param) 
